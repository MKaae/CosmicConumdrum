...
export function displayBoard(gameBoard, ghostRoutes) {
  const visualTiles = document.querySelectorAll("#background .tile");
  visualTiles.forEach(tile => tile.classList.remove('redpath'));
  visualTiles.forEach(tile => tile.classList.remove('floor'))
  ...
  for (let i = 0; i < gameBoard.GRID_HEIGHT; i++) {
    for (let j = 0; j < gameBoard.GRID_WIDTH; j++) {
      ...
      const isGhostRoute = ghostRoutes.some(route => 
        route.x === i && route.y === j
      );

      if(isGhostRoute){
        visualTile.classList.add('redpath');
      } else {
